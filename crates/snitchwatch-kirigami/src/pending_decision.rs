//! The pending-`AskRule` decision surface (Task 7).
//!
//! This is the single most safety-critical interaction in the app: the user
//! allows or denies a novel connection. The pure part of that decision — mapping
//! the four verdict buttons (allow/deny × once/always) and the scope selector
//! onto the bridge's typed [`ClientMessage::SetVerdict`] — lives here as a
//! Qt-free, unit-tested function. The cxx-qt `PendingDecision` wrapper is a thin
//! surface QML calls; it serialises the message and emits it via a signal.
//!
...
//! resolution path. No bridge code changes — this only builds the existing
//! `SetVerdict` message the WS protocol already defines.

use core::pin::Pin;
use cxx_qt_lib::QString;

use snitchwatch_bridge::ws_messages::{ClientMessage, VerdictAction, VerdictScope};

/// The four verdict buttons on the decision sheet.
...
impl VerdictChoice {
    ...
    pub fn action(self) -> VerdictAction {
        match self {
            Self::AllowOnce | Self::AllowAlways => VerdictAction::Allow,
            Self::DenyOnce | Self::DenyAlways => VerdictAction::Deny,
        }
    }

    /// Whether the decision should be remembered as a persistent rule.
...
pub mod qobject {
    ...
    extern "RustQt" {
        ...
        /// Submit a decision. `choice` is one of `allow_once` / `deny_once` /
        /// `allow_always` / `deny_always`; `scope` is `this_host` /
        /// `any_host_on_domain` / `any_host`. Malformed input is logged and
        /// dropped (never sends a wrong verdict).
        #[qinvokable]
        fn submit(
...
impl qobject::PendingDecision {
    fn submit(self: Pin<&mut Self>, row_id: &QString, choice: &QString, scope: &QString) {
        let row_id = row_id.to_string();
        let choice = choice.to_string();
        let scope = scope.to_string();
        match build_verdict_message(&row_id, &choice, &scope) {
...
mod tests {
    ...
    #[test]
    fn choice_tokens_map_to_action_and_remember() {
        assert_eq!(
            VerdictChoice::from_token("allow_once"),
            Some(VerdictChoice::AllowOnce)
        );
        assert_eq!(VerdictChoice::AllowOnce.action(), VerdictAction::Allow);
        assert!(!VerdictChoice::AllowOnce.remember());

        assert_eq!(VerdictChoice::DenyAlways.action(), VerdictAction::Deny);
        assert!(VerdictChoice::DenyAlways.remember());
    ...
    fn build_message_produces_expected_set_verdict() {
        let msg = build_verdict_message("r1", "deny_always", "any_host").unwrap();
        match msg {
    ...
    fn build_message_rejects_unknown_choice() {
        assert!(build_verdict_message("r1", "maybe", "this_host").is_none());
    }

    #[test]
    fn build_message_serializes_to_expected_json() {
        let msg = build_verdict_message("r9", "allow_once", "this_host").unwrap();
