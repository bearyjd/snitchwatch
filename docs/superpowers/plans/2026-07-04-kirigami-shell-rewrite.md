...
> - **Still outstanding:** old Tauri/`web/` removal (gated on real-use proof per non-goals). **Owner decision 2026-07-11: keep both `crates/snitchwatch-tauri/` and `web/` in the tree until a packaged release ships** — feature parity is done (incl. Task 7's fullscreen-focus verification), but "proven in real use" specifically means through a real packaged release, not a sandbox manual test. Don't remove either crate/dir before that regardless of parity status; re-raise with the owner if this criterion needs revisiting. **The blocklist `900-` band sort-order bug is fixed** (verified 2026-07-11, not just claimed): blocklist rules materialize into the `z00-blocklist:` alpha band, which sorts after every user-rule numeric prefix by construction, and a refresh purges any orphaned legacy `900-blocklist:` rules a pre-migration daemon might still hold (`owned_blocklist_rule_name_prefixes` covers both bands; `refresh_removes_legacy_band_rules`, `owned_prefixes_cover_current_and_legacy_bands`, and the two `classify_legacy_900_*` tests in `snitchwatch-bridge` all pass). See `blocklists/materializer.rs`'s "Legacy band migration" module doc for the full mechanism.
